/**
 * Raw inner SVG paths for every logo icon in lib/site-logo-icons.ts.
 * Generated from lucide-react's CJS bundle — never imports lucide-react
 * at runtime so this file is safe to use in server-only contexts like
 * app/icon.tsx (the metadata-image route), where lucide-react's "use client"
 * boundary causes a hard error when any component is called as a function.
 *
 * viewBox is always "0 0 24 24", stroke="white" stroke-width="2.5",
 * fill="none", stroke-linecap="round", stroke-linejoin="round" — those
 * attributes are added by the outer <svg> wrapper in icon.tsx.
 */
...
export function getIconSvgPaths(name: string | null | undefined): string {
  if (name && name in ICON_SVG_PATHS) return ICON_SVG_PATHS[name];
  return ICON_SVG_PATHS.Gamepad2;
}
